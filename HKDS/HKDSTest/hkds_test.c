...
bool monte_carlo_test()
{
	...
	for (i = 0; i < HKDSTEST_CYCLES_COUNT; ++i)
	{
		/* initialize the server with the client-ksn */
		hkds_server_initialize_state(&ss, &mdk, cs.ksn);
...
bool hkds_simd_encrypt_equivalence_test()
{
	...
	uint8_t keyp[HKDS_CACHX8_DEPTH][HKDS_BDK_SIZE] = { 0 };
...
{
	const uint8_t PID = 0x10;
	const uint8_t ad[HKDS_CACHX8_DEPTH][HKDS_MESSAGE_SIZE] = { 0xC0, 0xA8, 0x00, 0x01,
...
void hkds_test_run()
{
	...
	if (hkds_simd_encrypt_equivalence_test() == true)
	{
	...
	if (hkds_simd_authencrypt_equivalence_test() == true)
	{
	...
	if (hkds_parallel_encrypt_equivalence_test() == true)
	{
	...
	if (hkds_parallel_authencrypt_equivalence_test() == true)
	{
