...
size_t qsc_socket_receivefrom(qsc_socket* source, char destination[MAX_ADDRESS_LENGTH], size_t destlen, uint16_t port, uint8_t* output, size_t length, qsc_socket_receive_flags flag)
{
	int32_t res;
	int32_t len;
	...
	if (source->address_family == address_family_ipv4)
	{
		...
		res = recvfrom(source->connection, output, (int32_t)length, (int32_t)flag, (struct sockaddr*)&d, &len);

		inet_ntop(AF_INET, &d.sin_addr, astr, INET_ADDRSTRLEN);
		memcpy(destination, astr, len);
	}
	else
	{
		...
		res = recvfrom(source->connection, (char*)output, (int32_t)length, (int32_t)flag, (struct sockaddr*)&d, &len);
...
size_t qsc_socket_sendto(qsc_socket* source, const char* destination, size_t destlen, uint16_t port, const uint8_t* input, size_t length, qsc_socket_send_flags flag)
{
	int32_t res;
	...
	if (source->address_family == address_family_ipv4)
	{
		...
		res = sendto(source->connection, (const char*)input, (int32_t)length, (int32_t)flag, (struct sockaddr*)&d, sizeof(d));
	}
	else
	{
		struct sockaddr_in6 d;
